...
EVAL_RUN_NAME=${EVAL_RUN_NAME:-eval_table2_control}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"

EVAL_GPU_IDS=${EVAL_GPU_IDS:-0,1,2,3}
EVAL_TABLE2_NUM_ITER=${EVAL_TABLE2_NUM_ITER:-128}
...
echo "EVAL_TABLE2_NUM_ITER=${EVAL_TABLE2_NUM_ITER}"
...
ROWS=(
  "original_cosine"
  "finetuned_cosine"
  "planner_cosine_budget"
  "planner_score_budget"
)
...
for ROW in "${ROWS[@]}"; do
  ...
  RUN_DIR="${EVAL_ROOT}/${ROW}_iter${EVAL_TABLE2_NUM_ITER}"
  LOG_PATH="${LOG_DIR}/${EVAL_RUN_NAME}_${ROW}_iter${EVAL_TABLE2_NUM_ITER}.log"
  ...
  PIDS+=("$!")
  LABELS+=("${ROW}_iter${EVAL_TABLE2_NUM_ITER}_gpu${GPU_ID}")
