...
for POLICY in "${POLICIES[@]}"; do
  for NUM_ITER in "${NUM_ITERS[@]}"; do
    ...
    write_json_config "${RUN_DIR}/config.json" model_name="${MODEL_NAME}" model="${MODEL}" policy="${POLICY}" num_iter="${NUM_ITER}" data_path="${DATA_ROOT}" resume="${RESUME}" output_dir="${RUN_DIR}" sampling_policy="${POLICY}" pseudo_target_type="${PSEUDO_TARGET_TYPE}" budget_mode="${BUDGET_MODE}" cfg=2.9 num_images=50000 eval_bsz=256
    if "${CMD[@]}" 2>&1 | tee "${RUN_DIR}/eval.log" "${LOG_DIR}/eval_${POLICY}_iter${NUM_ITER}.log"; then
...
echo "To collect huge results:"
echo "python scripts_server/collect_main_results.py --root ${OUTPUT_ROOT} --models huge --skip_missing_models --output_prefix huge"
