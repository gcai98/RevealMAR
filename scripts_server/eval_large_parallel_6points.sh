...
check_path "${PRETRAIN_CKPT}" "pretrained large checkpoint"
if [[ -d "${PRETRAIN_CKPT}" ]]; then
  check_path "${PRETRAIN_CKPT}/checkpoint-last.pth" "pretrained large checkpoint-last.pth"
fi
...
for POLICY in "${POLICIES[@]}"; do
  if [[ "${POLICY}" = "planner" ]]; then
    check_path "${TRAIN_DIR}/checkpoint-last.pth" "trained large planner checkpoint"
  fi
done
