...
shutdown_if_requested_success() {
  if [[ "${AUTO_SHUTDOWN}" = "1" ]]; then
    echo "Training and parallel evaluation finished successfully. Shutting down..."
    sync
...
echo "EVAL_GPU_IDS=${EVAL_GPU_IDS:-0,1,2,3,4,5}"
...
bash "${SCRIPT_DIR}/eval_large_parallel_6points.sh"
