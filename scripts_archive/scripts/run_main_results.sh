...
# P0-1 Main Results Table runner.
# Defaults are smoke-test friendly and can be overridden from the environment.
...
TABLE_NAME="main"
TABLE_DIR="${OUTPUT_ROOT}/main_results"
mkdir -p "${TABLE_DIR}"
...
run_variant "PlanMAR-S Fixed Budget" "PlanMAR-S" "planner" "fixed" "${PLANMAR_RESUME}" "planner" "mixed_reveal" "hard"
run_variant "PlanMAR-S Score-derived Budget" "PlanMAR-S" "planner" "score-derived" "${PLANMAR_RESUME}" "planner" "mixed_reveal" "soft"

# TODO: Random Reveal / Confidence / Entropy are not exposed as sampling_policy
# choices in the current RevealMAR model. Add them here only if the model API
# supports them cleanly, without changing MAR sampling or training behavior.
