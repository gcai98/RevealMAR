...
REM ============================================================
REM P0 mixed-policy exposure ablation runner.
...
set "OUTPUT_ROOT=C:\caogang\RevealMAR\RevealMAR\p0_runs\mixed_policy_ablation"
...
set "PSEUDO_TARGET_TYPE=ref_mixed_reveal"
...
echo ============================================================
echo START MIXED-POLICY ABLATION: %DATE% %TIME%
...
echo Creating model-only init checkpoint for mixed-policy ablation.
...
call :RUN_RATIO ratio0p0 0.0
call :RUN_RATIO ratio0p25 0.25
call :RUN_RATIO ratio0p5 0.5
call :RUN_RATIO ratio0p75 0.75

echo ============================================================
echo Collector command
echo ============================================================
echo python collect_mixed_policy_ablation_results.py --root "%OUTPUT_ROOT%"
echo ============================================================
echo END MIXED-POLICY ABLATION: %DATE% %TIME%
echo ============================================================
exit /b 0

:RUN_RATIO
set "RATIO_NAME=%~1"
set "MIXED_POLICY_RATIO=%~2"
set "TRAIN_DIR=%OUTPUT_ROOT%\train_%RATIO_NAME%"
if not exist "%TRAIN_DIR%" mkdir "%TRAIN_DIR%"

set "TRAIN_CMD=python main_revealmar.py --model %MODEL% --img_size 256 --vae_path "%VAE_PATH%" --vae_embed_dim 16 --vae_stride 16 --patch_size 1 --data_path "%DATA_PATH%" --resume "%FINETUNE_INIT%" --output_dir "%TRAIN_DIR%" --batch_size %BATCH_SIZE% --epochs %EPOCHS% --save_last_freq 1 --eval_freq 999999 --num_workers %NUM_WORKERS% --diffloss_d %DIFFLOSS_D% --diffloss_w %DIFFLOSS_W% --num_sampling_steps %NUM_SAMPLING_STEPS% --diffusion_batch_mul %DIFFUSION_BATCH_MUL% --pseudo_target_type %PSEUDO_TARGET_TYPE% --ref_target_horizon %REF_TARGET_HORIZON% --ref_target_local_radius %REF_TARGET_LOCAL_RADIUS% --ref_target_mix_alpha %REF_TARGET_MIX_ALPHA% --ref_target_loss %REF_TARGET_LOSS% --ref_target_max_candidates %REF_TARGET_MAX_CANDIDATES% --mixed_policy_ratio %MIXED_POLICY_RATIO% --mixed_policy_ratio_schedule constant --log_mixed_policy_debug --log_mixed_policy_freq 20 --log_ref_target_debug --log_ref_target_freq 20 --log_revealmar_losses --log_revealmar_loss_freq 20 --dist_url env://"
echo !TRAIN_CMD! > "%TRAIN_DIR%\run_args.txt"
call :WRITE_CONFIG "%TRAIN_DIR%" "train" "%RATIO_NAME%" "%MIXED_POLICY_RATIO%" "" "%PSEUDO_TARGET_TYPE%" ""

echo ============================================================
echo TRAIN %RATIO_NAME% mixed_policy_ratio=%MIXED_POLICY_RATIO%
echo ============================================================
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { !TRAIN_CMD! 2>&1 | Tee-Object -FilePath '%TRAIN_DIR%\train.log' }"
if ERRORLEVEL 1 (
    echo [WARN] Training failed for %RATIO_NAME%. Continuing to eval attempts.
)

call :RUN_EVAL "%RATIO_NAME%" "%MIXED_POLICY_RATIO%" baseline hard
call :RUN_EVAL "%RATIO_NAME%" "%MIXED_POLICY_RATIO%" planner hard
call :RUN_EVAL "%RATIO_NAME%" "%MIXED_POLICY_RATIO%" planner soft
exit /b 0

:RUN_EVAL
set "RATIO_NAME=%~1"
set "MIXED_POLICY_RATIO=%~2"
set "SAMPLING_POLICY=%~3"
set "BUDGET_MODE=%~4"
if /I "%SAMPLING_POLICY%"=="baseline" (
    ...
    set "EVAL_PSEUDO=%PSEUDO_TARGET_TYPE%"
)
set "TRAIN_DIR=%OUTPUT_ROOT%\train_%RATIO_NAME%"
set "EVAL_DIR=%OUTPUT_ROOT%\eval_%RATIO_NAME%_%EVAL_VARIANT%"
...
call :WRITE_CONFIG "%EVAL_DIR%" "eval" "%RATIO_NAME%" "%MIXED_POLICY_RATIO%" "%EVAL_VARIANT%" "%EVAL_PSEUDO%" "%BUDGET_MODE%"

echo ------------------------------------------------------------
echo EVAL %RATIO_NAME% %EVAL_VARIANT%
echo ------------------------------------------------------------
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { !EVAL_CMD! 2>&1 | Tee-Object -FilePath '%EVAL_DIR%\eval.log' }"
if ERRORLEVEL 1 (
    echo [WARN] Eval failed for %RATIO_NAME% %EVAL_VARIANT%. Continuing.
)
exit /b 0
...
set "CFG_DIR=%~1"
set "CFG_PHASE=%~2"
set "CFG_RATIO_NAME=%~3"
set "CFG_RATIO=%~4"
set "CFG_EVAL_VARIANT=%~5"
set "CFG_PSEUDO=%~6"
set "CFG_BUDGET=%~7"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cfg = [ordered]@{ phase='%CFG_PHASE%'; ratio_name='%CFG_RATIO_NAME%'; mixed_policy_ratio=[double]'%CFG_RATIO%'; eval_variant='%CFG_EVAL_VARIANT%'; pseudo_target_type='%CFG_PSEUDO%'; budget_mode='%CFG_BUDGET%'; data_path='%DATA_PATH%'; source_checkpoint='%SOURCE_CHECKPOINT%'; finetune_init='%FINETUNE_INIT%'; output_dir='%CFG_DIR%'; model='%MODEL%'; batch_size=%BATCH_SIZE%; epochs=%EPOCHS%; num_images=%NUM_IMAGES%; eval_bsz=%EVAL_BSZ%; num_iter=%NUM_ITER%; cfg=%CFG%; ref_target_horizon=%REF_TARGET_HORIZON%; ref_target_local_radius=%REF_TARGET_LOCAL_RADIUS%; ref_target_mix_alpha=%REF_TARGET_MIX_ALPHA%; ref_target_loss='%REF_TARGET_LOSS%'; ref_target_max_candidates=%REF_TARGET_MAX_CANDIDATES%; timestamp=(Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ') }; $cfg | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 '%CFG_DIR%\config.json'"
exit /b 0
